...
if __name__ == "__main__":
    ...
    pred_cols = ['lr_pred', 'rf_pred', 'xgb_pred'] # PARAMETER <----------------
    ...
    print("Average")
    # Train
    avg_pred = np.mean(df_train[['rf_pred', 'rf_pred', 'xgb_pred']].values, axis=1)
    ...
    rf_pred = df_train.rf_pred.values
    avg_pred = (rf_pred + 2 * rf_pred + xgb_pred)/5
    ...
    rf_pred = df_train.rf_pred.rank().values
    avg_pred = (rf_pred + rf_pred + xgb_pred)/3
    print(metrics.roc_auc_score(targets, avg_pred))
    ...
    xgb_pred = df_train.xgb_pred.rank().values
    rf_pred = df_train.rf_pred.rank().values
    avg_pred = (rf_pred + rf_pred + xgb_pred)/5
    print(metrics.roc_auc_score(targets, avg_pred))
